...
public class Ai_RangeController : Ai_BasicController
{
    ...
    void Attack()
    {
        ...
        if (Physics.Raycast(transform.position+ (Vector3.up*attackHeight), transform.forward, out RaycastHit hit, rangeOfAttack))
        {
            if (hit.distance > 2)
            {
                ...
                ProjectileController.Spawn(projectile, transform.position + (Vector3.up * attackHeight),
                    Quaternion.Euler(
                        Vector3.Angle(playerTarget.position - transform.position, p),
